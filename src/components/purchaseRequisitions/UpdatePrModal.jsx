...
import { updatePurchaseReq } from '../../api/services/purchaseReqService';
import Swal from 'sweetalert2';

const UpdatePRModal = ({ isOpen, onClose, prData, onUpdateSuccess }) => {
  ...
  useEffect(() => {
    if (prData && isOpen) {
      ...
      setItems(prData.items?.length > 0 ? prData.items : [
  ...
  const handleAddItem = () => {
    setItems([...items, { item_name: '', specs: '', quantity: 1, uom: 'pcs', estimated_unit_price: 0 }]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updatePurchaseReq(prData.id, payload);
      onUpdateSuccess();
      onClose();
      ...
      console.error("Gagal update PR:", error);
      Swal.fire({
        title: 'ERROR!',
    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-neo-brutal-bg border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] w-full max-w-5xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex justify-between items-center border-b-4 border-black p-6 bg-white">
          <h2 className="text-2xl font-black uppercase tracking-tighter italic">Edit Dokumen PR</h2>
          <button onClick={onClose} className="p-2 border-4 border-black bg-neo-red hover:bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:translate-x-1 transition-all">
            <X size={24} />
          </button>
        </div>

        <div className="p-6 overflow-y-auto flex-1">
          <form id="update-pr-form" onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="bg-white border-4 border-black p-6 shadow-neo">
              <div className="flex justify-between items-end border-b-4 border-black pb-2 mb-4">
                ...
                <button type="button" onClick={handleAddItem} className="flex items-center gap-2 border-2 border-black bg-neo-green px-3 py-1 font-black uppercase text-sm shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 transition-all">
                ...
                {items.map((item, index) => (
                  <div key={index} className="flex flex-wrap md:flex-nowrap gap-4 items-start p-4 bg-gray-50 border-2 border-dashed border-black">
                    <div className="w-full md:w-3/12">
                      ...
                      <input type="text" required value={item.item_name} onChange={(e) => handleItemChange(index, 'item_name', e.target.value)} className={inputClass} />
                    </div>
                    <div className="w-full md:w-3/12">
                      <label className="block font-bold text-xs uppercase mb-1">Spesifikasi</label>
                      <input type="text" value={item.specs} onChange={(e) => handleItemChange(index, 'specs', e.target.value)} className={inputClass} />
                    </div>
                    <div className="w-1/3 md:w-1/12">
                      <label className="block font-bold text-xs uppercase mb-1">Qty</label>
                      <input type="number" min="1" required value={item.quantity} onChange={(e) => handleItemChange(index, 'quantity', e.target.value)} className={inputClass} />
                    </div>
                    <div className="w-1/3 md:w-2/12">
                      <label className="block font-bold text-xs uppercase mb-1">Satuan</label>
                      <select value={item.uom} onChange={(e) => handleItemChange(index, 'uom', e.target.value)} className={inputClass}>
                    ...
                    <div className="w-full md:w-2/12">
                      ...
                      <input type="number" min="0" required value={item.estimated_unit_price} onChange={(e) => handleItemChange(index, 'estimated_unit_price', e.target.value)} className={inputClass} />
                    </div>
                    <div className="w-auto flex items-end justify-center pt-5">
                      <button type="button" onClick={() => handleRemoveItem(index)} disabled={items.length === 1} className={`p-2 border-2 border-black ${items.length === 1 ? 'bg-gray-300' : 'bg-neo-red hover:bg-white'} shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all`}>
                        <Trash2 size={20} />
                      </button>
                    </div>
                  </div>
        ...
        <div className="border-t-4 border-black p-6 bg-white flex justify-between items-center">
          <div>
            ...
            <p className="text-3xl font-black text-neo-red">Rp {grandTotal.toLocaleString('id-ID')}</p>
          </div>
          <button type="submit" form="update-pr-form" disabled={loading} className="flex items-center gap-2 border-4 border-black bg-neo-blue px-8 py-4 font-black uppercase text-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 transition-all">
            <Save size={24} /> {loading ? 'Menyimpan...' : 'Simpan Perubahan'}
          </button>
        </div>
      </div>
    </div>
